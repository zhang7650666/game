<template>
    <div class="vme">
        <div class="emoji-container">
            <div class="emoji-category" ref="emojiCon">
                <span
                    ...
                    :style="emojiWidth"
                    :class="_emojiName(emoji)"
                    @click="emojiClick(emoji, i, $event)"
                >
                    <img
                        :src="
                            require('@/components/vue-mobile-emoji/common/emoji/' +
                                _emojiName(emoji))
                        "
                    />
                </span>
        ...
        <div class="bar-container">
            <div
                v-for="(item, index) in barControl"
...
<script>
import emojiData from "@/components/vue-mobile-emoji/common/emoji-data";
...
export default {
    ...
    computed: {
        emojiWidth() {
            const margin = 5;
            const width =
                (document.body.clientWidth - (this.perPage + 1) * margin) /
                this.perPage;
    ...
    methods: {
        ...
        _listenScroll() {
            ...
            this.$refs.emojiCon.addEventListener("scroll", () => {
                ...
                for (var i = 0; i < this.widthRange.length - 1; i++) {
                    if (
                        scrollLeft >= this.widthRange[i] &&
                        scrollLeft < this.widthRange[i + 1]
                    ) {
                        this.activedIndex = i;
                        break;
                    } else if (
                        scrollLeft >=
                        this.widthRange[this.widthRange.length - 1]
                    ) {
        ...
        barClick(item, index) {
            ...
            this.$refs.emojiCon.scrollLeft =
                index === 0 ? 0 : this.widthRange[index];
        },
        emojiClick(emoji, index, e) {
            ...
            this.$emit(
                "emojiSelected",
                emoji,
                require("@/components/vue-mobile-emoji/common/emoji/" + pngName)
            );
        }
    }
};
